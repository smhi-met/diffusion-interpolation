...
class DefaultTrainer(RunnerBase):
    ...
    def run(self):
        ...
        normalizer = (
            instantiate(
                self.normalizer_conf, stats=manifest.stats, var_dim=manifest.var_dim
            )
        ...
        profiler = instantiate(self.profiler_conf) if self.profiler_conf else None
        pltrainer_args = dict(self.pltrainer_conf or {})


        trainer = L.Trainer(
            **pltrainer_args, callbacks=callbacks, logger=logger, profiler=profiler
        )
